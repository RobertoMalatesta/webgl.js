...
var WebGL = (function (exports)
{
    ...
    function createBufferView(data, fields)
    {
        var GL      = WebGLConstants;
        var stride  = elementSize(fields);
    ...
    function interleave(fields, arrays, count)
    {
        var stride = elementSize(fields);
        var buffer = new ArrayBuffer(stride * count);
        var access = createBufferView(buffer, fields);
    ...
    function createImageView(data, unitType, levelOffset, levelSize)
    {
        var GL     = WebGLConstants;
    ...
    function imageStorageAttributes(width, height, pixelFormat, pixelType)
    {
        var GL     = WebGLConstants;
    ...
    /// VERTEX_BUFFER, ELEMENT_BUFFER, UNIFORM_BUFFER or GENERIC_BUFFER. The
    /// default is GENERIC_BUFFER.
    ...
    Buffer.prototype.specifyAttributes = function (args)
    {
        // ensure that all required arguments are specified.
        args              = args || {};
        args.usage        = args.usage      || 'GENERIC_BUFFER';
        args.updateType   = args.updateType || 'STREAM_DRAW';
    ...
    /// @param gl The WebGLRenderingContext.
    /// @return true if the WebGL resources were created successfully.
    Buffer.prototype.createBackingResources = function (gl)
    {
        if (!this.isBacked)
        {
            var bindQuery  = 0;
    ...
    Texture.prototype.specifyAttributes = function (args)
    {
        ...
        args               = args || {};
        args.type          = args.type       || '';
        args.usage         = args.usage      || 'TEXTURE_2D';
        args.target        = args.target     || 'TEXTURE_2D';
        args.format        = args.format     || 'RGBA';
        args.dataType      = args.dataType   || 'UNSIGNED_BYTE';
        args.wrapS         = args.wrapS      || 'CLAMP_TO_EDGE';
        args.wrapT         = args.wrapT      || 'CLAMP_TO_EDGE';
        args.magFilter     = args.magFilter  || 'NEAREST';
        args.minFilter     = args.minFilter  || 'NEAREST';
        args.compressed    = args.compressed || false;
        args.hasMipmaps    = args.hasMipmaps || false;
        args.levels        = args.levels     || [];
    ...
    /// @param gl The WebGLRenderingContext.
    /// @return true if the WebGL resources were created successfully.
    Texture.prototype.createBackingResources = function (gl)
    {
        if (!this.isBacked)
        {
    ...
    Renderbuffer.prototype.specifyAttributes = function (args)
    {
        ...
        args.format  = args.format || 'DEPTH_COMPONENT16';
        args.usage   = args.usage  || 'DEPTH_BUFFER';
    ...
    /// @param gl The WebGLRenderingContext.
    /// @return true if the WebGL resources were created successfully.
    Renderbuffer.prototype.createBackingResources = function (gl)
    {
        if (!this.isBacked)
        {
            var  GL     = WebGLConstants;
    ...
    /// @param gl The WebGLRenderingContext.
    /// @return true if the WebGL resources were created successfully.
    Framebuffer.prototype.createBackingResources = function (gl)
    {
        if (!this.isBacked)
        {
    ...
    BlendState.prototype.specifyAttributes = function (args)
    {
        // ensure that all required arguments are specified.
        args = args || {};
        args.blendEnabled      = args.blendEnabled      || false;
        args.constantColor     = args.constantColor     || [0.0, 0.0, 0.0, 0.0];
        args.sourceFactorColor = args.sourceFactorColor || 'ONE';
        args.sourceFactorAlpha = args.sourceFactorAlpha || 'ONE';
        args.targetFactorColor = args.targetFactorColor || 'ZERO';
        args.targetFactorAlpha = args.targetFactorAlpha || 'ZERO';
        args.functionColor     = args.functionColor     || 'FUNC_ADD';
        args.functionAlpha     = args.functionAlpha     || 'FUNC_ADD';
    ...
    /// @param gl The WebGLRenderingContext.
    /// @return true if the WebGL resources were created successfully.
    BlendState.prototype.createBackingResources = function (gl)
    {
    ...
    /// @summary Specifies the creation attributes of the raster state.
    /// @param args An object specifying creation attributes for the resource.
    /// @return true if the creation attributes were validated and stored.
    RasterState.prototype.specifyAttributes = function (args)
    {
        // ensure that all required arguments are specified.
        args = args || {};
        args.colorWrite            = args.colorWrite            || [true, true, true, true];
        args.cullingEnabled        = args.cullingEnabled        || false;
        args.cullFace              = args.cullFace              || 'BACK';
        args.windingOrder          = args.windingOrder          || 'CCW';
        args.scissorTestEnabled    = args.scissorTestEnabled    || false;
        args.scissorX              = args.scissorX              || 0;
        args.scissorY              = args.scissorY              || 0;
        args.scissorWidth          = args.scissorWidth          || 0;
        args.scissorHeight         = args.scissorHeight         || 0;
        args.lineWidth             = args.lineWidth             || 1.0;
        args.offsetFactor          = args.offsetFactor          || 0.0;
        args.offsetUnits           = args.offsetUnits           || 0.0;
        args.sampleCoverageEnabled = args.sampleCoverageEnabled || false;
        args.sampleAlphaToCoverage = args.sampleAlphaToCoverage || false;
        args.invertCoverage        = args.invertCoverage        || false;
        args.coverageValue         = args.coverageValue         || 1.0;
    ...
    /// @param gl The WebGLRenderingContext.
    /// @return true if the WebGL resources were created successfully.
    RasterState.prototype.createBackingResources = function (gl)
    {
        return true; // nothing to do for this type
    };
    ...
    var DrawContext = function ()
    {
        ...
        this.activeViewport     = {
            x      : 0,
            y      : 0,
            width  : (gl.drawingBufferWidth  || gl.canvas.width),
            height : (gl.drawingBufferHeight || gl.canvas.height),
            near   : 0.0,
            far    : 0.0
        };
        return this;
    ...
    /// @param gl The WebGLRenderingContext.
    /// @param viewport An object describing the viewport, as returned by
    /// the DrawContext.createViewport() function.
    /// @return The DrawContext.
    DrawContext.prototype.applyViewport = function (gl, viewport)
    {
    ...
    DrawContext.prototype.applyBlendState = function (gl, state)
    {
            ...
            active.targetFactorAlpha !== state.targetFactorAlpha)
        {
            gl.blendFuncSeparate(
                state.sourceFactorColor,
                state.targetFactorColor,
                state.sourceFactorAlpha,
                state.targetFactorAlpha);
    ...
    DrawContext.prototype.bindFramebuffer = function (gl, resource)
    {
        ...
        if (resource)
        {
            if (resource !== bound)
            {
                ...
                if (resource.drawBuffers.length > 1)
                {
                    gl.drawBuffersWEBGL(resource.drawBuffers);
                }
                else if (gl.drawBuffersWEBGL)
                {
                    gl.drawBuffersWEBGL(this.defaultDrawBuffer);
                }
    ...
    DrawContext.prototype.unbind = function (gl)
    {
        ...
        if (gl && !gl.isContextLost())
        {
            for (int i = 0, n = TextureSlots.length; i < n; ++i)
            {
    ...
    exports.Constants              = WebGLConstants;
